...
function AdminControls({ onSetTopic, onSwitchSides, onStartDebate, debateStarted, onUpdateTeams }) {
  ...
  useEffect(() => {
    localStorage.setItem('debateTeamA', JSON.stringify(teamA));
    localStorage.setItem('debateTeamB', JSON.stringify(teamB));
    onUpdateTeams(teamA, teamB);
  }, [teamA, teamB, onUpdateTeams]);
  ...
  const handleAddDebater = (e) => {
    e.preventDefault();
    if (newDebaterName.trim()) {
      if (selectedTeam === 'A') {
        setTeamA(prev => [...prev, newDebaterName.trim()]);
      } else {
        setTeamB(prev => [...prev, newDebaterName.trim()]);
      }
      setNewDebaterName('');
    }
  };

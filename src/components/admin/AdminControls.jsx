...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (savedClassroom) {
      try {
        ...
        console.error('Error parsing saved classroom:', error);
        // Clear corrupted data
        localStorage.removeItem('currentClassroom');
        actions.setError('Session data was corrupted. Please create a new session.');
      }
    }
  }, []);
  ...
  const loadClassroomData = async (classroomId) => {
    if (!classroomId) {
      console.warn('loadClassroomData called without classroomId');
      return;
    }
    ...
    try {
      ...
      let debateData, teamsData, studentsData;
      
      try {
        debateData = await getDebateData(classroomId);
      } catch (error) {
        console.warn('Failed to load debate data:', error);
        debateData = {
      ...
      try {
        teamsData = await getTeams(classroomId);
      } catch (error) {
        console.warn('Failed to load teams data:', error);
        teamsData = { teamA: [], teamB: [] };
      }
      
      try {
        studentsData = await getClassroomStudents(classroomId);
      } catch (error) {
        console.warn('Failed to load students data:', error);
        studentsData = [];
        setLoadingError('Could not load student list. Some features may not work properly.');
      }

      // Set the loaded data
      if (debateData) {
        actions.setTopic(debateData.topic || "Is technology making us less social?");
      ...
      if (teamsData) {
        actions.setTeams({
          teamA: Array.isArray(teamsData.teamA) ? teamsData.teamA : [],
          teamB: Array.isArray(teamsData.teamB) ? teamsData.teamB : []
        });
      }
      ...
      actions.setError('Failed to load classroom data. Please try refreshing the page.');
      actions.setLoading(false);
    }
  };
  ...
  const handleCreateClassroom = async (classroomData) => {
    try {
      ...
      actions.setError(error.message || 'Failed to create classroom');
    }
  };
  ...
  const handleUpdateTopic = async () => {
    ...
    try {
      ...
      actions.setError(error.message || 'Failed to update topic');
    }
  };
  ...
  const handleStartDebate = async () => {
    ...
    try {
      ...
      actions.setError(error.message || 'Failed to start debate');
    }
  };
  ...
  const handleSwitchSides = async () => {
    ...
    try {
      ...
      actions.setError(error.message || 'Failed to switch sides');
    }
  };

  const handleRemoveStudent = async (admissionNumber) => {
    if (!activeClassroom || !admissionNumber) return;
    ...
    try {
      ...
      actions.setError(error.message || 'Failed to remove student');
    }
  };
  ...
  const handleClearAllTeams = async () => {
    ...
    try {
      ...
      actions.setError(error.message || 'Failed to clear teams');
    }
  };
      ...
      <div className="admin-dashboard">
        ...
        {loadingError && (
          <div className="warning-message">
            {loadingError}
            <button onClick={() => setLoadingError('')}>×</button>
          </div>
        )}
                ...
                {state.teamA && state.teamA.length > 0 ? (
                  state.teamA.map((student, index) => (
                    <li key={student.admissionNumber || index} className="team-member">
                      ...
                      <button
                        onClick={() => handleRemoveStudent(student.admissionNumber || student)}
          ...
          {students.length > 0 && (
            <div className="team-actions">
              <div className="balance-info">
                ...
                <span className={Math.abs((state.teamA?.length || 0) - (state.teamB?.length || 0)) <= 1 ? 'balanced' : 'unbalanced'}>
                  {Math.abs((state.teamA?.length || 0) - (state.teamB?.length || 0)) <= 1 ? 'Balanced' : 'Needs Balancing'}
                </span>
              </div>
          ...
          {!state.debateStarted ? (
            <div className="start-section">
              ...
              <button
                ...
                disabled={(state.teamA?.length || 0) === 0 || (state.teamB?.length || 0) === 0}
              >
                <Play size={16} />
                Start Debate
              </button>
              {((state.teamA?.length || 0) === 0 || (state.teamB?.length || 0) === 0) && (
            ...
            <div className="active-debate">
              ...
              <div className="vote-display">
                <div className="vote-item">
                  ...
                  <span className="vote-count">{state.votes?.switch || 0}</span>
                </div>
                <div className="vote-item">
                  <span className="vote-label">Keep Current:</span>
                  <span className="vote-count">{state.votes?.dontSwitch || 0}</span>
                </div>
              </div>
